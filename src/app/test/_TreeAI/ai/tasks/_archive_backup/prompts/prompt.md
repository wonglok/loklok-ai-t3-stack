Please write me write an execution plan in json for vibe coding and a typescript client with vercel ai sdk. 
The execution plan has phases and tasks

For each task, it should have:
1. task name
2. task system prompt (with [ReplacerToken])
3. task user prompt (with [ReplacerToken])
4. task output files
4. files required / dependencies required
4. injection parameters list, for each item: { key: [ReplacerToken], readFile: '[filepath]'  }

I have a existing prompt like this, please integrate the prompt into the json output:
...
Input Requirements:
${userPrompt} (Example: "I want a simple app where users can sign up, log in, create tasks with titles and descriptions, mark tasks as done, and see a list of their tasks.")
